...
int main(int argc, char *argv[])
{
  ...
  char reductionFileName[34];
  char mergedFileName[34];
  ...
  strcat(originalFileName, ".bmp");

  strcpy(reductionFileName, argv[1]);
  strcat(reductionFileName, "_reduction.bmp");

  strcpy(mergedFileName, argv[1]);
  strcat(mergedFileName, "_merged.bmp");

  // The input command has two or three strings, the 2nd one is the name of the input file.
  fptr = fopen(originalFileName, "rb"); // Open the input file using binary mode.
  if (fptr == NULL)
  {                                                        // If the file pointer is empty, it means the file does not exist.
    printf("File %s does not exist.\n", originalFileName); // Print error message.
  ...
  // Print the original image file head.
  ...
  fptr = fopen(reductionFileName, "wb"); // Open the output file using binary mode.
  if (fptr == NULL)
  {
    printf("File %s does not exist.\n", reductionFileName);
    return 1;
  ...
  fptr = fopen(mergedFileName, "wb"); // Open the output file using binary mode.
  if (fptr == NULL)
  {
    printf("File %s does not exist.\n", originalFileName); // Print error message.
    return 1;                                              // Terminate the program.
