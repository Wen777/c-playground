 ...
 // Input the colored image file.
  ...
  for (i = 0; i < header.Height / 2; i++) { // Split the image in to the lower half and the upper half.
    for (j = 0; j < header.Width; j++) { // Go through all colums.
      ...
      // Swap blue level.
      temp = imageData[k_low]; // Store the lower-half value to temporary memory.
      imageData[k_low] = imageData[k_up]; // Copy the topper-half value to the lower-half.
      imageData[k_up] = temp; // Copy the old lower-half value to the topper-half.
      // Swap the green level.
      temp = imageData[k_low+1]; // Store the lower value to temporary memory.
      imageData[k_low+1] = imageData[k_up+1]; // Copy the topper-half value to the lower-half.
      imageData[k_up+1] = temp; // Copy the old lower-half value to the topper-half.
      // Swap the red level.
      temp = imageData[k_low+2]; // Store the lower value to temporary memory.
      imageData[k_low+2] = imageData[k_up+2]; // Copy the topper-half value to the lower-half.
      imageData[k_up+2] = temp; // Copy the old lower-half value to the topper-half.
    }
  }
  
  // If the input command has three strings, use the 3rd one as the output file name;
  // otherwise, use the default file name "horizontal_mirror_reflection.bmp".
  if (argc==3) { // User provided output file name.
    ...
    fptr = fopen("horizontal_mirror_reflection.bmp", "wb"); // Open the output file using binary mode.
